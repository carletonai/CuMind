"""
Random name generator for experiment runs.

Portions derived from Docker project (https://github.com/moby/moby)
Copyright 2013-2018 Docker, Inc.
Licensed under Apache License 2.0

This module is officially "frozen" - no new additions will be accepted.
"""
...
# Scientists and hackers for naming
right: list[str] = [
...
# forced to use random, alt: jax / np
import random  # noqa: E402


def get_name(retry: int = 0) -> str:
    """Generate a random run name using JAX PRNG.

    Args:
        retry: If > 0, adds a random digit suffix for collision handling

    Returns:
        String in format "adjective_surname" or "adjective_surname3" if retry > 0
    """
    while True:
        # Use JAX PRNG for deterministic randomness
        left_idx = random.randint(0, len(left) - 1)
        right_idx = random.randint(0, len(right) - 1)

        name = f"{left[left_idx]}_{right[right_idx]}"

        # Special case: Steve Wozniak is not boring
        if name == "boring_wozniak":
            continue

        # Add retry suffix if needed for collision handling
        if retry > 0:
