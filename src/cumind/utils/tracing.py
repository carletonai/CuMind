...
def start_trace(workspace_path: Path) -> None:
    ...
    # Check if profiler is already running to avoid duplicate traces
    try:
        ...
        if "Profile has already been started" in str(e):
            # Profiler is already running, skip starting a new one
            return
        else:
