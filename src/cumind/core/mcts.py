...
class MCTS:
    ...
    def __init__(self, network: "CuMindNetwork"):
    ...
    def search(self, root_hidden_state: chex.Array, add_noise: bool = True) -> np.ndarray:
        ...
        visit_counts = np.zeros(cfg.env.action_space_size)
    ...
    def _simulate(self, root: Node) -> None:
        ...
        while node.is_expanded():
            action = node.select_child(cfg.mcts.c_puct)
    ...
    def _add_exploration_noise(self, root: Node) -> None:
        ...
        noise = np.random.dirichlet([cfg.mcts.dirichlet_alpha] * num_actions)

        # Mix with existing priors
        for i, (_, child) in enumerate(root.children.items()):
            child.prior = child.prior * (1 - cfg.mcts.exploration_fraction) + noise[i] * cfg.mcts.exploration_fraction
