...
class CuMindNetwork(nnx.Module):
    ...
    def get_prediction_network(self) -> Callable[[chex.Array], Tuple[chex.Array, chex.Array]]:
        return self.prediction_network

    def set_prediction_network(self, new_pred: Callable[[chex.Array], Tuple[chex.Array, chex.Array]]) -> None:
        self.prediction_network = new_pred

    def get_target_prediction_network(self) -> Callable[[chex.Array], Tuple[chex.Array, chex.Array]]:
        return self.target_prediction_network

    def set_target_prediction_network(self, new_pred: Callable[[chex.Array], Tuple[chex.Array, chex.Array]]) -> None:
        self.target_prediction_network = new_pred
    ...
    def initial_inference(self, observation: chex.Array, use_target: bool = False) -> Tuple[chex.Array, chex.Array, chex.Array]:
        ...
        if use_target:
            ...
            # log.debug("Using target prediction network for initial inference")
        else:
            policy_logits, value = self.prediction_network(hidden_state)
            # log.debug("Using online prediction network for initial inference")
