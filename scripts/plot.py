...
def parse_log_file(filepath):
    episode_re = re.compile(
        r"(?P<time>\d{2}:\d{2}:\d{2} [AP]M) - INFO - Episode (?P<ep>\d+): Reward=\s*(?P<reward>[-+]?[0-9]*\.?[0-9]+), Length=\s*\d+, Loss=(?P<loss>[-+]?[0-9]*\.?[0-9]+), Memory=.*"
    )
    policy_loss_re = re.compile(
        r"(?P<time>\d{2}:\d{2}:\d{2} [AP]M) - INFO - Step\s+\d+: train/policy_loss = (?P<loss>[-+]?[0-9]*\.?[0-9]+)"
    )
    reward_loss_re = re.compile(
        r"(?P<time>\d{2}:\d{2}:\d{2} [AP]M) - INFO - Step\s+\d+: train/reward_loss = (?P<loss>[-+]?[0-9]*\.?[0-9]+)"
    )
    value_loss_re = re.compile(
        r"(?P<time>\d{2}:\d{2}:\d{2} [AP]M) - INFO - Step\s+\d+: train/value_loss = (?P<loss>[-+]?[0-9]*\.?[0-9]+)"
    )
    total_loss_re = re.compile(
        r"(?P<time>\d{2}:\d{2}:\d{2} [AP]M) - INFO - Step\s+\d+: total_loss = (?P<loss>[-+]?[0-9]*\.?[0-9]+)"
    )
...
def main():
    ...
    # Reward plot
    plot_and_save(
        data['episode_times'], data['rewards'],
        "Reward", "Reward",
        os.path.join(outdir, "reward.png")
    )

    # Episode loss plot
    if data['loss']:
        plot_and_save(
            data['episode_times'], data['loss'],
            "Episode Loss", "Loss",
    ...
    if data['policy_loss']:
        plot_and_save(
            data['policy_loss_times'], data['policy_loss'],
            "Policy Loss", "Loss",
            os.path.join(outdir, "policy_loss.png")
        )

    # Reward loss plot
    if data['reward_loss']:
        plot_and_save(
            data['reward_loss_times'], data['reward_loss'],
            "Reward Loss", "Loss",
            os.path.join(outdir, "reward_loss.png")
        )

    # Value loss plot
    if data['value_loss']:
        plot_and_save(
            data['value_loss_times'], data['value_loss'],
            "Value Loss", "Loss",
            os.path.join(outdir, "value_loss.png")
        )

    # Total loss plot
    if data['total_loss']:
        plot_and_save(
            data['total_loss_times'], data['total_loss'],
            "Total Loss", "Loss",
            os.path.join(outdir, "total_loss.png")
        )
